...
class IncomeListTableViewController : UITableViewController {
    ...
    var fetchedResultsController: NSFetchedResultsController<Income> {
        let fetchResultsController = NSFetchedResultsController(fetchRequest: Income.incomeFetchRequest(), managedObjectContext: AppDelegate.persistentContainer.viewContext, sectionNameKeyPath: nil, cacheName: nil)
